...
/*   Updated: 2025/03/19 17:20:38 by flvoicu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	in_philo(t_aux *aux, char **av, int i)
{
	...
	p = &aux->vars[i].philo;
	p->max_philo = aux->philo_n;
	p->philo_n = i + 1;
	p->die = ft_atoi(av[2], aux);
	p->eat = ft_atoi(av[3], aux);
	p->sleep = ft_atoi(av[4], aux);
	p->last_eat = 0;
	p->times = -1;
	if (ft_atoi(av[5], aux))
		p->times = ft_atoi(av[5], aux);
	p->fork_l = &aux->vars[i].fork;
	p->fork_r = &aux->vars[(i + 1) % aux->philo_n].fork;
	p->dead = &aux->dead;
	p->check = &aux->check;
	p->write = &aux->write;
	p->track = &aux->track;
